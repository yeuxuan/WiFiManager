...
"<form action='/restart' method='get'><button>复位</button></form><br/>\n",// MENU_RESTART
"<form action='/exit'    method='get'><button>退出</button></form><br/>\n",  // MENU_EXIT
"<form action='/erase'   method='get'><button class='D'>擦 除</button></form><br/>\n", // MENU_ERASE
...
const char HTTP_FORM_WIFI[]        PROGMEM = "<label for='s'>WIFI名称</label><input id='s' name='s' maxlength='32' autocorrect='off' autocapitalize='none' placeholder='{v}'><br/><label for='p'>WIFI密码</label><input id='p' name='p' maxlength='64' type='password' placeholder='{p}'><input type='checkbox' onclick='f()'> 显示密码";
...
const char HTTP_SAVED[]            PROGMEM = "<div class='msg'>正在保存凭据<br/>尝试将 ESP 连接到网络。<br />如果失败，请重新连接到 AP 以重试</div>";
...
const char HTTP_STATUS_ON[]        PROGMEM = "<div class='msg S'><strong>已经连接</strong> to {v}<br/><em><small>with IP {i}</small></em></div>";
const char HTTP_STATUS_OFF[]       PROGMEM = "<div class='msg {c}'><strong>没有连接</strong> to {v}{r}</div>"; // {c=class} {v=ssid} {r=status_off}
const char HTTP_STATUS_OFFPW[]     PROGMEM = "<br/>密码错误"; // STATION_WRONG_PASSWORD,  no eps32
const char HTTP_STATUS_OFFNOAP[]   PROGMEM = "<br/>AP 没有发现";   // WL_NO_SSID_AVAIL
const char HTTP_STATUS_OFFFAIL[]   PROGMEM = "<br/>无法连接"; // WL_CONNECT_FAILED
const char HTTP_STATUS_NONE[]      PROGMEM = "<div class='msg'>没有设置AP</div>";
...
const char HTTP_UPDATE_FAIL[] PROGMEM = "<div class='msg D'><strong>更新失败!</strong><Br/>请重启设备再次尝试</div>";
...
const char HTTP_INFO_aboutver[]     PROGMEM = "<dt>WiFi管理</dt><dd>{1}</dd>";
